...
async function api(url, options = {}) {
  ...
  try {
    ...
    data = { error: body || `请求失败 ${res.status}` };
  }
...
function dungeonCardHtml(dungeon) {
  ...
  return `<article class="dungeon-card ${expanded ? 'expanded' : ''} ${isComplete ? 'complete' : ''}">
    ...
    <section class="dungeon-metrics">
      ...
      <div class="dungeon-metric efficiency"><span>基础性价比分数</span>${costEfficiencyHtml}</div>
...
function ghostHuntHtml(data) {
  const ghostHunt = data.ghostHunt || {};
  const summaries = new Map((ghostHunt.ringSummaries || []).map(row => [Number(row.ring), row]));
  const completeRings = [...summaries.values()].filter(row => row.experience != null).length;
  const totals = ['experience', 'cash', 'reserveGold'].reduce((result, field) => {
    result[field] = [...summaries.values()].reduce((sum, row) => sum + (Number(row[field]) || 0), 0);
    return result;
  }, {});
  const totalValue = (value, label) => `<div class="ghost-hunt-total-metric"><span>${label}</span><strong>${value.toLocaleString('zh-CN')}</strong></div>`;
  $('#ghostHuntTotal').innerHTML = `${totalValue(totals.experience, '十环总经验')}${totalValue(totals.cash, '十环总现金')}${totalValue(totals.reserveGold, '十环总储备')}<div class="ghost-hunt-total-metric completion"><span>完成度</span><strong>${completeRings}/10 环</strong><small>储备金不折算现金</small></div>`;
  $('#ghostHuntRings').innerHTML = Array.from({ length: 10 }, (_, index) => {
    const ring = index + 1;
    const summary = summaries.get(ring);
    const sampleCount = summary?.sampleCount || 0;
    const value = (number, label) => number == null
      ? `<span>${label} 待收集</span>`
      : `<span>${label} <b>${Number(number).toLocaleString('zh-CN', { maximumFractionDigits: 2 })}</b></span>`;
    return `<article class="ghost-hunt-ring ${ring === 10 ? 'tenth' : ''}"><strong>第${ring}环</strong><div>${value(summary?.experience, '经验')}${value(summary?.cash, '现金')}${value(summary?.reserveGold, '储备')}</div><small>${sampleCount ? `${sampleCount} 次实测` : '暂无实测'}</small></article>`;
  }).join('');
  const firstRing = (ghostHunt.ringRecords || []).find(record => Number(record.ring) === 1);
  $('#ghostHuntEvidence').innerHTML = firstRing
    ? `首环明细：基础经验 ${Number(firstRing.baseExperience || 0).toLocaleString('zh-CN')} · 盛福经验 ${Number(firstRing.fortuneExperience || 0).toLocaleString('zh-CN')} · 现金 ${Number(firstRing.cash || 0).toLocaleString('zh-CN')} · 储备 ${Number(firstRing.reserveGold || 0).toLocaleString('zh-CN')}（不折算）`
    : '尚未收集抓鬼环数明细。';
  const detailToggle = $('#ghostHuntDetailToggle');
  if (detailToggle && !detailToggle.dataset.bound) {
    detailToggle.dataset.bound = 'true';
    detailToggle.addEventListener('click', () => {
      const details = $('#ghostHuntDetails');
      const expanded = details.dataset.expanded !== 'true';
      details.dataset.expanded = String(expanded);
      details.hidden = !expanded;
      detailToggle.setAttribute('aria-expanded', String(expanded));
      detailToggle.textContent = expanded ? '收起十环明细' : '展开十环明细';
    });
  }
  const rewardNotes = ghostHunt.rewardNotes || [];
  $('#ghostHuntDrops').innerHTML = rewardNotes.length
    ? rewardNotes.map(note => `<span class="ghost-hunt-reward-note">${esc(note)}</span>`).join('')
    : '<span class="dungeon-summary-empty">待补充</span>';
  const details = $('#ghostHuntDetails');
  const expanded = details?.dataset.expanded === 'true';
